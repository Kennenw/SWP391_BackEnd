...
namespace BookingBad.BLL.Services
{
    ...
    {
        public PagedResult<CourtDTO> GetCourtsPage(SortContent sortContent, int pageNumber, int pageSize);
    ...
    public class CourtServices : ICourtServices
    {
        ...
        public CourtDTO GetCourtById(int id)
        {
            ...
            return new CourtDTO
            {
                ...
                Status = court.Status,
                Image = court.Image,
                Amenities = _unitOfWork.AmenityCourtRepo.GetByCourtId(id).Select(ac => new AmenityDTO
                {
                    AmenityId = ac.Amenity.AmenityId,
                    Description = ac.Amenity.Description,
                    Status = ac.Status
                }).ToList(),
                CourtNumbers = _unitOfWork.CourtNumberRepo.GetCourtNumberId(id).Select(cn => new CourtNumberDTO
                {
        ...
        public void CreateCourt(CourtDTO courtDTO)
        {
            ...
            foreach (var amenityDTO in courtDTO.Amenities)
            {
                var amenityCourt = new AmenityCourt
                {
                ...
                _unitOfWork.AmenityCourtRepo.Create(amenityCourt);
            }

            foreach (var courtNumberDTO in courtDTO.CourtNumbers)
            {
                var courtNumber = new CourtNumber
                {
                    Number = courtNumberDTO.Number,
                    CourtId = court.CourtId,
                    Status = courtNumberDTO.Status
                };

                _unitOfWork.CourtNumberRepo.Create(courtNumber);
            }
        ...
        public void UpdateCourt(int id, CourtDTO courtDTO)
        {
            ...
            _unitOfWork.SaveChanges();
            var existingAmenities = _unitOfWork.AmenityCourtRepo.GetByCourtId(court.CourtId);
            foreach (var existingAmenity in existingAmenities)
            {
                _unitOfWork.AmenityCourtRepo.Remove(existingAmenity);
            }
            ...
            // Update court numbers
            var existingCourtNumbers = _unitOfWork.CourtNumberRepo.GetCourtNumberId(court.CourtId);
            foreach (var existingCourtNumber in existingCourtNumbers)
            {
                _unitOfWork.CourtNumberRepo.Remove(existingCourtNumber);
            }
        ...
        {
            var court = _unitOfWork.CourtRepo.GetById(id);
            if (court == null) throw new InvalidOperationException("Court not found");

            // Remove amenities
            var amenities = _unitOfWork.AmenityCourtRepo.GetByCourtId(id);
            foreach (var amenity in amenities)
            {
                _unitOfWork.AmenityCourtRepo.Remove(amenity);
            }

            // Remove court numbers
            var courtNumbers = _unitOfWork.CourtNumberRepo.GetCourtNumberId(id);
            foreach (var courtNumber in courtNumbers)
            {
                _unitOfWork.CourtNumberRepo.Remove(courtNumber);
            }
            _unitOfWork.CourtRepo.Remove(court);
            _unitOfWork.SaveChanges();
        }
